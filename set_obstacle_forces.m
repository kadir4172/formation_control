  ...
  if(obstacle_number ~= 0) 
    for j = 1 : 1 : n
      for i = 1 : 1 : array_length
        ...
        force_matrix(1,6,j) = force_matrix(1,6,j) + ((X(j) - obstacle_global(1,i)) / (distance_to_point^4));
        force_matrix(2,6,j) = force_matrix(2,6,j) + ((Y(j) - obstacle_global(2,i)) / (distance_to_point^4));
      end

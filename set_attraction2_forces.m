  ...
  for i = 1 : 1 : n
    if(inside_outside_array(i) ~= 1) %eger agent shape icerisinde degilse hesaplansin
      for j = 1 : 1 : array_length
        ...
        force_matrix(1,5,i) = force_matrix(1,5,i) + ((formation_x(j) - X(i)) / (distance_to_point)^3);
        force_matrix(2,5,i) = force_matrix(2,5,i) + ((formation_y(j) - Y(i)) / (distance_to_point)^3);
      end

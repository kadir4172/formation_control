  ...
  for i = 1 : 1 : n
    if(offline_inside_outside_array(i) ~= 1) %eger agent shape icerisinde degilse hesaplansin
      for j = 1 : 1 : array_length
        offline_force_matrix(1,1,i) = offline_force_matrix(1,1,i) + (formation_x(j) - X(i)) * real_time_scale;
        offline_force_matrix(2,1,i) = offline_force_matrix(2,1,i) + (formation_y(j) - Y(i)) * real_time_scale;
      end

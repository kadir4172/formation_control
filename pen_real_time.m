...
if estat_pulsacio % sol buton basili ise
    % boto pulsat
    ...
    if ((x ~= x_pen(end)) || (y ~= y_pen(end))) %en son alinan datadan farkli bir dataysa
        ...
        if (distance > pen_length)
         ...
         temp_y = y_pen(end) + (((y - y_pen(end)) * pen_length) / distance);
         x = temp_x;
         y = temp_y;
         % punt seguent
         x_pen = [x_pen x];
...
else %sol button basili degilse
    
    if ((x < 0.1) && (y < 0.1)) || ((x > 0.9) && (y < 0.1)) %kirmizi karelerin oldugu yerler
...
plot_index = evalin('base', 'plot_index');
if  (plot_index == 1)
    assignin('base', 'formation_sari_x', x_pen);
    assignin('base', 'formation_sari_y', y_pen);
elseif(plot_index == 2)
    assignin('base', 'formation_kirmizi_x', x_pen);
    assignin('base', 'formation_kirmizi_y', y_pen);
elseif(plot_index == 3)
    assignin('base', 'formation_yesil_x', x_pen);
    assignin('base', 'formation_yesil_y', y_pen);
elseif(plot_index == 4)
    assignin('base', 'formation_mavi_x', x_pen);
    assignin('base', 'formation_mavi_y', y_pen);
elseif(plot_index == 5)
    assignin('base', 'formation_pembe_x', x_pen);
    assignin('base', 'formation_pembe_y', y_pen);
end

plot_index = plot_index + 1;
assignin('base', 'plot_index', plot_index);
assignin('base', 'formation_', plot_index);

  ...
  for i = 1 : 1 : n
    if(inside_outside_array(i) ~= 0) %eger agent, shape disarisinda degilse hesaplansin
      for j = 1 : 1 : array_length
        ...
        force_matrix(1,2,i) = force_matrix(1,2,i) + (((X(i) - formation_x(j)) / (distance_to_point)) / (distance_to_point - agents_radius(i))^2);
        force_matrix(2,2,i) = force_matrix(2,2,i) + (((Y(i) - formation_y(j)) / (distance_to_point)) / (distance_to_point - agents_radius(i))^2);
      end
